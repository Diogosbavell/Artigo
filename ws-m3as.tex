...
\markboth{Borges, Diogo S.; 
	Costa-Félix, R.P.B. and Lava, Deise D.}{A Methology Proposal for Aging Analisys in Electronic Ciruits}
	...
	\begin{center}
		\resizebox{125mm}{!}{
			{\begin{tabular}{@{}ccccccccc@{}}\toprule
					\\
					Component & Failure Mode& $\eta \hspace{2pt}(H)$&$\beta<1^{b}$& $\beta=1^{b}$&$\beta>1^{b}$&$\gamma$\hspace{2pt}($H$)\\\\
					\colrule
					\\\\
					\multicolumn {1}{l}{Resistor}& \multicolumn {1}{l}{Accuracy Loss}&$M=1540; U_{std}=34^{a}$&$M=0.3;U_{std}=0.05$&$M=1; U_{std}=0.02$&$M=170; U_{std}=0.05$&0.1\\\\
					\multicolumn {1}{l}{Variable Resistor}&\multicolumn {1}{l}{Position Failure}&$M=1535; U_{std}=25^{a}$&$M=0.3;U_{std}=0.05$&$M=1;U_{std}=0.02$&$M=150; U_{std}=0.02$&0.1\\\\
					\multicolumn {1}{l}{Integrated Circuit}&\multicolumn {1}{l}{Operation Failure}&$^{c}$&$M=0.3;U_{std}=0.05$&$M=1;U_{std}=0.02$&$M=175;U_{std}=0.04$&0.1\\\\
					\botrule\\
		\end{tabular}}}
	\end{center}
